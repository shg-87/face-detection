...
class FaissHNSW:
    ...
    def add_embeddings(self, new_embeddings, new_metadata):
        ...
        if new_embeddings is None:
            raise ValueError("new_embeddings cannot be None.")
        if new_metadata is None:
            raise ValueError("new_metadata cannot be None.")
        ...
        if emb.shape[1] != self.dim:
            raise ValueError(f"Embedding dimensionality mismatch: expected {self.dim}, got {emb.shape[1]}.")

        if len(new_metadata) != emb.shape[0]:
            raise ValueError(
                f"Number of embeddings ({emb.shape[0]}) and metadata entries ({len(new_metadata)}) do not match."
            )

        # For cosine, normalize each vector to unit length before adding
        if self.metric == "cosine":
            norms = np.linalg.norm(emb, axis=1, keepdims=True)
            mask = norms.squeeze() > 0
            emb2 = emb.copy()
            emb2[mask] = emb[mask] / norms[mask]
            emb = emb2
